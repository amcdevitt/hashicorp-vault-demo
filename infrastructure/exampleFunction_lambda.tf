...
resource "aws_lambda_function" "exampleFunction_function" {
  ...
  s3_key    = aws_s3_bucket_object.exampleFunction_lambda.key

  runtime = "nodejs14.x"
  handler = var.exampleFunction_function_handler

  source_code_hash = filemd5("../build/exampleFunction.zip")
...
# Variables
#####################################
variable "exampleFunction_function_name" { type = string } # default="exampleFunction"
