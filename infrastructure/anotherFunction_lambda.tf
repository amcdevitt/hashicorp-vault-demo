...
resource "aws_lambda_function" "anotherFunction_function" {
  ...
  s3_key    = aws_s3_bucket_object.anotherFunction_lambda.key

  runtime = "nodejs14.x"
  handler = var.anotherFunction_function_handler

  source_code_hash = filemd5("../build/anotherFunction.zip")
...
# Variables
#####################################
variable "anotherFunction_function_name" { type = string } # default="anotherFunction"
